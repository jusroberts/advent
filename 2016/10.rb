...
input = "bot 127 gives low to output 1 and high to bot 180
...
def parse_bot_transfer(line)
  hash = {}
  parse = line[3..-1]
  parse = parse.split(" gives low to ")
  hash[:bot_num] = parse[0].to_i
  parse = parse[1].split(" and high to ")
  hash[:low_to_type] = parse[0].split(" ")[0]
  hash[:low_to] = parse[0].split(" ")[1].to_i
  hash[:high_to_type] = parse[1].split(" ")[0]
  hash[:high_to] = parse[1].split(" ")[1].to_i
  hash
end

def parse_value_additions(line)
  hash = {}
  parse = line[5..-1].split(" goes to bot ")
  hash[:value] = parse[0].strip.to_i
  hash[:bot_num] = parse[1].to_i
  hash
end

input.split("\n").each do |line|
  if line[0,3] == "bot"
    @bot_transfers << parse_bot_transfer(line)
  else
    @value_additions << parse_value_additions(line)
  end
end
@bot_transfers.reverse!
@value_additions.reverse!
@output_bins = []
@bots = []

def parse_things
  while @bot_transfers.length != 0
    if !do_bot_transfers
      do_value_addition
    end
  end
...
def do_bot_transfers
  (0..@bot_transfers.length - 1).each do |i|
    return true if do_bot_transfer(i)
  end
  return false
end

def do_bot_transfer(num)
  return false if @bots[@bot_transfers[num][:bot_num]] == nil || @bots[@bot_transfers[num][:bot_num]].length != 2
  bt = @bot_transfers[num]
  @bot_transfers.delete_at(num)
  bot_num = bt[:bot_num]
  values = @bots[bot_num].sort
  @bots[bot_num] = []
  puts bot_num if values[1] == 61 && values[0] == 17
  if bt[:high_to_type] == "output"
    give_to_output(bt[:high_to], values[1])
  else
    give_to_bot(bt[:high_to], values[1])
  end

  if bt[:low_to_type] == "output"
    give_to_output(bt[:low_to], values[0])
  else
    give_to_bot(bt[:low_to], values[0])
  end
  return true
end

def give_to_bot(bot_num, value)
  @bots[bot_num] = [] if @bots[bot_num] == nil
  @bots[bot_num] << value
end

def give_to_output(output_num, value)
  @outputs[output_num] = [] if @outputs[output_num] == nil
  @outputs[output_num] << value
end

def do_value_addition
  value_addition = @value_additions.pop
  give_to_bot(value_addition[:bot_num], value_addition[:value])
end
