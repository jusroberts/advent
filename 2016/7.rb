...
parse_input.map! do |a| 
  b = [[],[]]
  a.each_with_index do |string, i|
    b[i % 2] << string
  end
  b
end
def contains_abba?(string)
  (0..string.length - 4).each do |i|
    return true if string[i,2] == string[i+2,2].reverse && string[i] != string[i+1]
  end
  return false
end

def check_each_for_abba(array)
  array.each do |s|
    if contains_abba?(s)
      return true
    end
...
def allows_tls(input)
  return 1 if check_each_for_abba(input[0]) && !check_each_for_abba(input[1])
  return 0
end
count = 0
parse_input.each do |input|
  count += allows_tls(input)
end
...
def get_all_aba(string_array)
  aba_strings = []
  string_array.each do |string|
    (0..string.length-3).each do |i|
      if string[i] == string[i + 2]
        aba_strings << string[i, 3]
      end
    end
  end
  return aba_strings
...
def aba_match_bab(aba, bab)
  aba.each do |string|
    test_bab = "#{string[1]}#{string[0]}#{string[1]}"
    return 1 if bab.include?(test_bab)
  end
  return 0
end
count = 0
parse_input.each do |input|
  aba_strings = get_all_aba(input[0])
  bab_strings = get_all_aba(input[1])
  count += aba_match_bab(aba_strings, bab_strings)
end
puts count