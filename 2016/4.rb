...
rooms = input.split("\n").map do |a|
  hash = {}
  hash[:checksum] = a[-6..-2]
  hash[:encryption] = a[0..-11].gsub("-", "")
  hash[:value] = a[-11..-8].to_i
  hash
end
...
def is_valid?(room)
  ...
  room[:encryption].each_char do |c|
    if alphabet[c] != nil
      alphabet[c] += 1
    else
      alphabet[c] = 1
  ...
  sorted_alphabet = alphabet.map do |k, v|
    [k, v]
  end.sort do |a, b|
    b[1] <=> a[1]
  end
  ...
  potentials = [[]]
  sorted_alphabet.each do |t|
    if potentials.last.empty?
      potentials[-1] << t
    elsif potentials.last[0][1] == t[1]
      potentials[-1] << t
    else
      potentials << []
      potentials[-1] << t
  ...
  potential = potentials.map {|a| a.sort}.flatten.join
  return potential[0..4] == room[:checksum] ? room[:value] : 0
end
...
rooms.each do |r|
  ...
  count += value
  if value != 0
    puts "#{caesar_cipher(r[:encryption], -r[:value]).join} #{r[:value]}"
  end
end
p count
