...
inputs.each do |input|
  ...
  elsif input_array[1] == "column"
    x = input_array[2].split("=")[1].to_i
    y_shift = input_array.last.to_i
    temp_array = (0..height-1).map do |i|
      grid[i][x]
    end
    temp_array.rotate!(-y_shift)
    temp_array.each_with_index do |val, i|
      grid[i][x] = val
    end
  else
    x_shift = input_array.last.to_i
    y = input_array[2].split("=")[1].to_i
    grid[y].rotate!(-x_shift)
  end
end
count = 0
grid.each do |line|
